...
class Node @Since("1.2.0") (
  ...
  private[tree] def subtreeToString(indentFactor: Int = 0): String = {
    ...
    if (isLeaf) {
      prefix + s"return ${predict.predict};\n"
//      prefix + "break;\n"
    } else {
      prefix + s"if ${splitToString(split.get, left = true)} {\n" +
        leftNode.get.subtreeToString(indentFactor + 1) + prefix+ s"}\n" +
        prefix + s"else if ${splitToString(split.get, left = false)}{\n" +
        rightNode.get.subtreeToString(indentFactor + 1) + prefix + s"}\n"
    }
  }
